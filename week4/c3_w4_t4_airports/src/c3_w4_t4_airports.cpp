...
public:
	...
	using Items = array<pair<TAirport, size_t>, static_cast<uint32_t>(TAirport::Last_)>;
...
private:
	array<pair<TAirport, size_t>, static_cast<uint32_t>(TAirport::Last_)> airports;
};

void TestMoscow() {
	enum class MoscowAirport {
		VKO, SVO, DME, ZIA, Last_
	};

	const vector<MoscowAirport> airports = { MoscowAirport::SVO, MoscowAirport::VKO, MoscowAirport::ZIA, MoscowAirport::SVO, };
...
void TestMostPopularAirport() {
	...
	for (auto &x : dayly_flight_report) {
		x = { static_cast<SmallCountryAirports>(gen_airport(rnd)), static_cast<SmallCountryAirports>(gen_airport(rnd)) };
	}
...
int main() {
	...
	LOG_DURATION("Total tests duration");   // Total tests duration: 2045 ms
