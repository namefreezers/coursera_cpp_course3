...
ForwardIterator max_element_if(ForwardIterator first, ForwardIterator last, UnaryPredicate pred) {
	auto res_it = find_if(first, last, pred);
	auto cur_it = res_it;

	while (cur_it != last) {
		++cur_it;
		cur_it = find_if(cur_it, last, pred);
		if (cur_it == last) {
			break;
		}
		if (*res_it < *cur_it) {
...
void TestSeveralMax() {
	...
	Assert(max_element_if(text.begin(), text.end(), IsCapitalized()) == max_iterator, "Expect thr first \"Two\"");
}
